...
// bit 0=1 enable SPI
  ...
  IOMUX->SECCFG.PINCM[PB12INDEX] = 0x00000081;  // GPIO output, LCD !RST
  IOMUX->SECCFG.PINCM[PB13INDEX] = 0x00000081;  // GPIO output, LCD LITE
  GPIOB->DOE31_0 |= (1<<12) | (1<<13);    // PB12 is LCD !, LITE = 1
  GPIOB->DOUTSET31_0 = (1<<12) | (1<<13); // !RST = 1, RS=1, LITE = 1
  SPI_Reset();
}
